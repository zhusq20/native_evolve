    ...
    def __init__(
        ...
        Args:
            ...
            truncation_method: How to truncate trajectory when too long.
                "middle" (default): remove from the middle, preserving beginning (context) and end (results).
                "first": remove from the beginning (oldest items first).
                "last": remove from the end (newest items first).
        """
        self.model = model
        ...
        for attempt in range(max_attempt):
            try:
                ...
                if attempt < max_attempt - 1 and len(current_user_input) > 0:
                    # Remove one trajectory item based on truncation method
                    if self.truncation_method == "middle":
                        removed_item = current_user_input.pop(len(current_user_input) // 2)
                    elif self.truncation_method == "first":
                        removed_item = current_user_input.pop(0)
                    else:  # "last"
                        removed_item = current_user_input.pop()
