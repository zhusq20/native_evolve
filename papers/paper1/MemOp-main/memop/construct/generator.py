...
from agent.llm import LiteLLMAPI
from utils.logger import logger
from utils.json_util import read_from_json, save_to_json, read_from_jsonl
from construct.prompts import MEMORY_PROMPT
from construct.conv_converter import parse_single_instance


class MemoryGenerator:
    def __init__(self, args):
        """
        Generate memory snippets for raw coding trajectories.
        """
        # Source data (trajectories to be reorganized to memory)
        self.args = args
        ...
        self.reorganized_memories = {}  # dict of subdicts, each subdict for an instance, key: instance_id, value: reorganized memory dict
    ...
    def _init_reorganized_memories_and_cost(self):
        # Reorganized memory
        save_path = os.path.join(self.save_dir, "all_memory_candidates.json")
    ...
    def _parse_model_response(self, response: str):
        ...
        if response and response.strip():
            ...
            # if len(response) > 2000:  # adjust limit as needed
    ...
    def memory_reorganization(self):
        ...
        for progress_idx in tqdm(range(self.instance_num)):
            # Read raw trajectory
            # raw_history = self.trajectories[progress_idx]["history"]  # list of dicts
            ...
            raw_trajectory = parse_single_instance(instance_trajectory_dir)  # list of dicts
            ...
            if f"raw_trajectory_{self.raw_trajectory_idx}" in self.reorganized_memories[instance_id]:
                ...
                self.reorganized_memories[instance_id][f"raw_trajectory_{self.raw_trajectory_idx}"] = {
                    ...
                    "trajectory_with_memory": None,    # {<candidate_id>: <trajectory_with_memory>},
